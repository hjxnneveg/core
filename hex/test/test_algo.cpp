...
DEFINE_TEST(foreach_band) {
    struct { qrs west; qrs east; int count; } ends[] = {
        {{ 0,-2}, { 0,-2}, 1},
        {{-1,-1}, { 1,-2}, 2},
        {{-2, 0}, { 2,-2}, 3},
        {{-1, 0}, { 1,-1}, 2},
    ...
    foreach_band(5, [&](qrs west, qrs east, int count) {
        TEST_LT(i, countof(ends));
        TEST_EQ(west, ends[i].west);
        TEST_EQ(east, ends[i].east);
        TEST_EQ(count, ends[i].count);
        i++;
    });

    TEST_EQ(i, countof(ends));
}
